...
<?php
    ...
    if(isset($_POST['add_member'])){
        ...
        if($img_r =='inv_img'){
            ...
            $q="INSERT INTO `team_detalis3`(`name`,`picture`) VALUES (?,?)";
    ...
    if(isset($_POST['rem_member'])){
        ...
        $pre_q="SELECT * FROM `team_detalis3` WHERE `sr_no`=?";
        $res=select($pre_q,$values,'i');
        $img=mysqli_fetch_assoc($res);

        if(deleteImage($img['picture'],ABOUT_FOLDER)){
            $q="DELETE FROM `team_detalis3` WHERE `sr_no`=?";
